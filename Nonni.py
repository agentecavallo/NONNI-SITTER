...
# --- NUOVA FUNZIONE PER LE DATE ---
def ottieni_date_settimana():
    """Calcola le date dal Lunedì al Venerdì della settimana corrente"""
    oggi = datetime.now()
    # Troviamo il lunedì della settimana corrente (0=Lunedì, 1=Martedì...)
    ...
    for i, nome in enumerate(nomi_giorni):
        ...
        # Formato: Lunedì 2 Marzo 2026
        # Usiamo un dizionario per i mesi in italiano perché il sistema potrebbe essere in inglese
    ...
    return date_formattate, oggi.strftime("%d/%m/%Y")

# 2. FUNZIONI MEMORIA (Invariate ma corazzate)
...
date_settimana, data_oggi_testo = ottieni_date_settimana()

# ==========================================
# 3. INTERFACCIA A SCHEDE
# ==========================================
sch_nonni, sch_genitori = st.tabs(["🚕 Vista Nonni", "⚙️ Modifica Genitori"])

# --- VISTA NONNI ---
with sch_nonni:
    st.title("🚕 Il Taxi dei Nipoti")
    st.info(f"📅 Oggi è il: **{data_oggi_testo}**")
    st.markdown("---")

    for giorno_chiave, impegni in programma.items():
        # Recuperiamo la data dinamica (es: Lunedì 2 Marzo 2026)
        data_completa = date_settimana[giorno_chiave]
        
        # Evidenziamo se è il giorno corrente
        if datetime.now().strftime("%d %m") in data_completa:
            st.subheader(f"🌟 {data_completa.upper()} (OGGI)")
        else:
...
with sch_genitori:
    ...
    # Qui usiamo la data completa anche nel menu a tendina per chiarezza
